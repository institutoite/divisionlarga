...
class DivisionCalculator {
    ...
    iniciarDivision() {
        ...
        this.currentStep = -1; // No mostrar ningún paso al iniciar
        this.steps = [];
        this.calcularTodosLosPasos();
        this.mostrarInterfaz();
        this.mostrarSoloDivisionBase(); // Mostrar cuadrícula base visual
    ...
    calcularTodosLosPasos() {
        ...
        while (i < dividendoArr.length) {
            ...
            // --- NUEVA LÓGICA: registrar ceros en el cociente por cada cifra bajada que no alcanza ---
            while (dividendoParcial < this.divisor && i + 1 < dividendoArr.length) {
                // EXCEPCIÓN: Si es el primer paso (aún no hay dígitos en el cociente), NO agregar cero ni registrar paso
                if (cociente.length === 0) {
                    i++;
                    dividendoParcial = dividendoParcial * 10 + dividendoArr[i];
                    continue;
                }
                // Agregar un 0 al cociente (solo después del primer dígito)
                cociente += '0';
                filaCociente = cociente.split('');
                // Registrar el paso con el dividendo parcial actual
                ...
                // Mostrar la siguiente cifra bajada
                let nextDigit = dividendoArr[i + 1];
                let colBajada = startCol + partialStr.length;
                ...
                const step = {
                    ...
                    droppedDigitCol: i + 1, // la cifra que se baja
            ...
            // Ahora realizar la división si corresponde
    ...
    // Muestra solo la división base: dividendo, divisor y la L, todo lo demás en blanco
    mostrarSoloDivisionBase() {
        this.gridContainer = document.querySelector('.grid-container');
        if (!this.gridContainer) return;
        // Obtener los valores actuales de dividendo y divisor
        const dividendoStr = this.dividendo ? this.dividendo.toString() : '';
        const divisorStr = this.divisor ? this.divisor.toString() : '';
        const maxCols = (dividendoStr.length || 4) + (divisorStr.length || 2) + 2;
        // Limpiar cuadrícula
        ...
        for (let i = 0; i < (dividendoStr.length || 4); i++) {
            const td = document.createElement('td');
            td.className = 'cuadro-celda';
            td.textContent = dividendoStr[i] || '';
            if (i === (dividendoStr.length || 4) - 1) {
                td.style.borderRight = '4px solid black';
            }
            tr0.appendChild(td);
        }
        for (let i = 0; i < (divisorStr.length || 2); i++) {
            const td = document.createElement('td');
            td.className = 'cuadro-celda';
        ...
        for (let i = (dividendoStr.length || 4) + (divisorStr.length || 2); i < maxCols; i++) {
            const td = document.createElement('td');
            td.className = 'cuadro-celda';
            td.style.borderBottom = '4px solid black';
            tr0.appendChild(td);
        ...
        // Mostrar panel de explicación y ocultar resultado
        document.getElementById('explanation-panel').style.display = 'block';
        document.getElementById('result-panel').style.display = 'none';
        // Mensaje inicial en la explicación
        document.getElementById('explanation-line-1').innerHTML = '<strong>Haz clic en Adelante para ver el primer paso de la división larga.</strong>';
        document.getElementById('explanation-line-2').innerHTML = '';
        document.getElementById('explanation-line-3').innerHTML = '';
    }

    // Renderiza la cuadrícula tipo cuaderno con la L y los números alineados
    renderizarCuadricula() {
        if (this.currentStep === -1) {
            this.mostrarSoloDivisionBase();
            return;
        }
    ...
    actualizarBotones() {
        ...
        siguienteBtn.disabled = (this.currentStep >= this.steps.length - 1) && (this.currentStep !== -1);
        resolverTodoBtn.disabled = this.currentStep >= this.steps.length - 1;
        
        // Actualizar estilos de los botones
        atrasBtn.classList.toggle('disabled', this.currentStep <= 0);
        siguienteBtn.classList.toggle('disabled', (this.currentStep >= this.steps.length - 1) && (this.currentStep !== -1));
    ...
    reiniciar() {
        ...
        // this.mostrarSoloDivisionBase(); // Mostrar cuadrícula base visual
        // this.actualizarBotones();
        // No limpiar campos ni ocultar paneles ni vaciar los pasos
    }
...
//         for(let i=0;i<step.dividendo.length-1;i++){ 
//             const tr = document.createElement('tr');
...
//         table.appendChild(tr2);
//         this.gridContainer.appendChild(table);
//         // Ocultar paneles de explicación y resultado
//        // ... existing code ...
//         this.gridContainer.appendChild(table);
//         // Mostrar panel de explicación y ocultar resultado
//         document.getElementById('explanation-panel').style.display = 'block';
...
//             this.actualizarBotones();
//         }
//           // Al final, cuando actualices las explicaciones:
//         const explicacionActual = obtenerExplicacionPaso(pasoActual);
        
//         // Actualizar contenido
//         document.getElementById('explanation-line-1').textContent = explicacionActual.linea1;
//         document.getElementById('explanation-line-2').textContent = explicacionActual.linea2;
//         document.getElementById('explanation-line-3').textContent = explicacionActual.linea3;
...
//     mostrarExplicacion(linea1, linea2, linea3) {
//         // Actualizar el contenido
//         document.getElementById('explanation-line-1').textContent = linea1;
//         document.getElementById('explanation-line-2').textContent = linea2;
//         document.getElementById('explanation-line-3').textContent = linea3;
        
//         // Ejecutar animación de escritura
//         Typear(40, 200); // 40ms por carácter, 200ms entre líneas
//     }
